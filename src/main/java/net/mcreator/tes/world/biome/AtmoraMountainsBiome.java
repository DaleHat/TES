...
public class AtmoraMountainsBiome extends TesModElements.ModElement {
	public static Biome biome;
	public AtmoraMountainsBiome(TesModElements instance) {
		super(instance, 103);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.SNOW).category(Biome.Category.ICY).depth(1f).scale(0.2f).temperature(0f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("tes:atmora_mountains"));
			}
		}
	}
	@Override
